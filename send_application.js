...
(async () => {
  ...
  //-----------------------------------1.Login-----------------------------------------------
  //1.Auto Login
  await page.fill('input[name="session_key"]', 'test_user@gmail.com');
  ...
  for (let job of jobListings) {
    ...
    await page.getByLabel('Email addressEmail address').selectOption('test_user@gmail.com');
    await page.getByLabel('Mobile phone number').fill('1234567890');
    ...
    //----------------Template-2
    await page.setInputFiles('input[type="file"]', 'Sample Resume.pdf');
