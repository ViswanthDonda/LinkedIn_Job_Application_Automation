...
const fs = require('fs');
const { answersDatabase, handleNewQuestion, getMostSimilarQuestion} = require('./utils_Numeric.js');
const { answerDropDown } = require('./utils_DropDown');
const { answerBinaryQuestions} = require('./utils_Binary.js');
...
(async () => {
  ...
  for (let job of jobListings) {
    ...
    if (emailLabel) {
      ...
      await page.selectOption(`#${emailInputId}`, 'test_user@gmail.com');
    }
